            ...
            for (let j = 0; j < $scope.contents.row; j++) {
                for (let k = 0; k < $scope.contents.col; k++) {
                    ...
                    if ( abr_id > 1 && abr_id != 6 ) {
                        $scope.players[$scope.playerCount].enablerlABR(true);
                    }
    
                    // uncomment this block if you want to change the buffer size that dash tries to maintain
                    $scope.players[$scope.playerCount].setBufferTimeAtTopQuality(60);
                    $scope.players[$scope.playerCount].setStableBufferTime(60);
                    $scope.players[$scope.playerCount].setBufferToKeep(60);
                    $scope.players[$scope.playerCount].setBufferPruningInterval(60);
                    ...
                    if ( abr_id == 6 ) { //BOLA
                        $scope.players[$scope.playerCount].enableBufferOccupancyABR(true);
                    }
                    $scope.players[$scope.playerCount].setAbrAlgorithm(abr_id);
